...
public class ControllerFactory {
	 ...
	 */
	public ControllerAction createController(final HttpRequest httpRequest) {

		final String pathInfo = httpRequest.getPath();
		...
		if (StringUtils.isEmpty(realPath)) {
			return getDefaultControllerAction(application);
		}

		final String[] pathParts = realPath.split("/");

		final ControllerAction controllerAction = createControllerAction(application, pathParts);
	...
	private ControllerAction decideWithPathParts(final Module moduleContext, final ControllerClass controllerClass, final String[] pathParts) {
		...
		case 1:
			...
			try {
				final ControllerClass defaultControllerClass = moduleContext.getDefaultControllerClass();
	...
	private Pair<ControllerClass, Object> getDefaultController(final Module module) {
		...
		final ControllerClass controllerClass = module.getDefaultControllerClass();
